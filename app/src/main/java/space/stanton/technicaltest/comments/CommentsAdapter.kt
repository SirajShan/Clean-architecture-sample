...
class CommentsAdapter(private val items: ArrayList<CommentsItem>?, val onItemClick: (Int) -> Unit) :
    RecyclerView.Adapter<CommentsAdapter.PostViewHolder>() {
    ...
    override fun onBindViewHolder(holder: PostViewHolder, position: Int) {
        holder.itemView.findViewById<TextView>(R.id.title).text = items?.get(position)?.name
